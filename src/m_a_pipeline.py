...
import json
from src.config import SLEAK_INTERACTIONS_PATH, SLEAK_PROFILES_PATH
...
def companies_interactions(f_profiles, f_interactions):
    ...
    sleak_model_graph = list(map(create_node, sleak_profiles)) + list(map(create_link, sleak_interactions))
    # save the file with tracking date
    with open(f"sleak_model_{datetime.now()}", "w") as final:
